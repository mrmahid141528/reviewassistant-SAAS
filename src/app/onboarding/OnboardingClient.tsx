...
export type OnboardingClientProps = {
    brandSettings?: { platformName?: string, logoUrl?: string | null }
}
...
export function OnboardingClient({ brandSettings }: OnboardingClientProps) {
    const router = useRouter()

    const [step, setStep] = useState(1)
    const [isSubmitting, setIsSubmitting] = useState(false)
    const [error, setError] = useState<string | null>(null)
    const [intendedPlanId, setIntendedPlanId] = useState<string | null>(null)

    React.useEffect(() => {
        setIntendedPlanId(localStorage.getItem('intendedPlanId'))
    }, [])
    ...
    const renderStep1 = () => (
        <div className="flex flex-col items-center text-center animate-in fade-in zoom-in-95 duration-500">
            {brandSettings?.logoUrl ? (
                <div className="flex items-center justify-center mb-6 h-16 w-auto">
            ...
            <h1 className="text-3xl font-bold tracking-tight mb-4">Welcome to {brandSettings?.platformName || 'Smart Review Assistant'} 👋</h1>
            ...
            <div className="w-full flex justify-center mt-6">
                <Button onClick={nextStep} className="w-full sm:w-auto px-10 h-12 text-lg rounded-xl shadow-lg shadow-primary/25 hover:scale-[1.02] transition-all">
        ...
        <div className="flex flex-col items-center text-center animate-in fade-in zoom-in-95 duration-500 pt-4">
            ...
            {intendedPlanId ? (
                <div className="flex flex-col gap-3 w-full max-w-md mt-4">
                    <Button
                        className="w-full h-12 text-lg font-bold bg-primary text-primary-foreground hover:bg-primary/90 shadow-lg shadow-primary/25"
                        onClick={() => router.push(`/checkout/${intendedPlanId}?cycle=yearly`)}
                    >
    ...
    const STEPS = [
        { title: 'Business Basics', number: 1 },
        { title: 'Business Details', number: 2 },
        { title: 'Review Preferences', number: 3 },
        { title: 'QR Code Setup', number: 4 },
        { title: 'QR Ready', number: 5 }
    ]

    const renderStepper = () => (
        <div className="mb-10 w-full max-w-3xl mx-auto px-4 mt-6 animate-in fade-in duration-500">
            <div className="flex justify-between items-center relative">
                {/* Background Track */}
                <div className="absolute left-[10%] right-[10%] top-4 h-[2px] bg-muted -z-10" />
                {/* Active Track */}
                <div
                    className="absolute left-[10%] top-4 h-[2px] bg-primary transition-all duration-700 -z-10"
                ...
                {STEPS.map((s) => {
                    ...
                    return (
                        <div key={s.number} className="flex flex-col items-center gap-3">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold transition-all duration-500 ${isActive ? 'bg-primary text-primary-foreground shadow-md ring-4 ring-primary/20' : 'bg-background text-muted-foreground border-2 border-muted'}`}>
                                {s.number}
                            </div>
                            <span className={`text-[11px] sm:text-xs font-semibold hidden sm:block ${isCurrent ? 'text-primary' : isActive ? 'text-foreground' : 'text-muted-foreground'}`}>{s.title}</span>
                        </div>
                    )
                })}
            </div>
    ...
    return (
        <div className="w-full">
            ...
            <div className="bg-white/80 backdrop-blur-xl sm:border border-border/50 sm:shadow-2xl rounded-3xl sm:p-12 transition-all duration-300">
                {step === 1 && renderStep1()}
                {step === 2 && renderStep2()}
                {step === 3 && renderStep3()}
                {step === 4 && renderStep4()}
